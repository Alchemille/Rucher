...
def get_species():
    ruchers = Rucher.query.filter_by(user=current_user.id).all()
    list_species = [(i.specie, i.specie) for i in ruchers]
...
def search_event():
    ...
    if ruche != "":
        ...
        events = events.filter(
            (Event.ruche == ruche_of_interest.id) | ((Event.rucher == rucher_of_interest) & (Event.ruche == None)))
